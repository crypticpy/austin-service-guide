...
def _format_known_facts(session: IntakeSession) -> str:
    ...
    if profile.immediate_needs:
        parts.append(f"needs={profile.immediate_needs}")
    ...
    if profile.has_children is not None:
        parts.append(f"has_children={profile.has_children}")
    if profile.veteran_status is not None:
        parts.append(f"veteran={profile.veteran_status}")
    if profile.has_disability is not None:
        parts.append(f"has_disability={profile.has_disability}")
    if profile.is_outdoor_worker:
        parts.append("outdoor_worker=true")
    if profile.has_ac is False:
        parts.append("has_ac=false")
    if profile.has_chronic_conditions:
        parts.append("chronic_conditions=true")
    if profile.is_refugee_or_immigrant:
        parts.append("refugee_or_immigrant=true")
